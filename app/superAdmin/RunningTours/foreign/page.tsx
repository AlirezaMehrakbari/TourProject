...
const ResidencePanel = () => {
    ...
    //@ts-ignore
    const fetchTourForeignSuper = async (): Promise<{}> => {
    ...
    const {data, isLoading} = useQuery({
        queryKey: ['TourForeignSuper'],
        queryFn: () => fetchTourForeignSuper()
    })
            ...
            <div className='w-full justify-start py-8 flex gap-x-[7%]'>
                ...
                <div className='w-full flex flex-col items-center gap-y-2 '>
                    ...
                    <table className='w-[90%] mx-auto text-center '>
                        ...
                        {/*@ts-ignore*/}
                        {data.filter(item=>{
                            return item.type === 'خارجی'
                        })
                            .map(report => {
                            return (
                                <tr className='even:bg-[#F0F0F0] odd:bg-[#E9E4E4]'>
                                    <td className='py-4'>{report.tourManager}</td>
                                    <td>{report.origin}</td>
